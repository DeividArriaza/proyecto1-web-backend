...
// GameStore expone las operaciones de persistencia para juegos sobre un
...
// ListOptions agrupa los filtros y la paginación que admite GET /games.
...
// List retorna los juegos que cumplen `opts` junto con el total que coincide
// con el filtro `q` (sin aplicar paginación).
func (s *GameStore) List(ctx context.Context, opts ListOptions) ([]models.Game, int, error) {
	if opts.Page < 1 {
		opts.Page = 1
	}
	if opts.Limit < 1 {
	...
	listSQL := fmt.Sprintf(`
		SELECT id, title, genre, status, hours_played, total_hours,
	...
	for rows.Next() {
		var g models.Game
		if err := rows.Scan(
			&g.ID, &g.Title, &g.Genre, &g.Status, &g.HoursPlayed,
			&g.TotalHours, &g.ImagePath, &g.CreatedAt, &g.UpdatedAt,
		); err != nil {
	...
	if err := rows.Err(); err != nil {
		return nil, 0, fmt.Errorf("iter games: %w", err)
	}
	return out, total, nil
}

func (s *GameStore) GetByID(ctx context.Context, id int) (*models.Game, error) {
	const q = `
		SELECT id, title, genre, status, hours_played, total_hours,
		       image_path, created_at, updated_at
		  FROM games
...
// Insert crea un juego y devuelve el registro completo (con timestamps e id
// asignados por la DB).
func (s *GameStore) Insert(ctx context.Context, in models.GameInput) (*models.Game, error) {
	const q = `
		INSERT INTO games (title, genre, status, hours_played, total_hours)
		     VALUES ($1, $2, $3, COALESCE($4, 0), $5)
		  RETURNING id, title, genre, status, hours_played, total_hours,
		            image_path, created_at, updated_at`
...
func (s *GameStore) Update(ctx context.Context, id int, in models.GameInput) (*models.Game, error) {
	const q = `
		UPDATE games
		   SET title        = $1,
		       genre        = $2,
...
func (s *GameStore) Delete(ctx context.Context, id int) error {
...
func (s *GameStore) SetImagePath(ctx context.Context, id int, path string) error {
	tag, err := s.pool.Exec(ctx,
		"UPDATE games SET image_path = $1, updated_at = NOW() WHERE id = $2",
		path, id,
	)
	if err != nil {
...
func (s *GameStore) CurrentImagePath(ctx context.Context, id int) (*string, error) {
	var path *string
	err := s.pool.QueryRow(ctx, "SELECT image_path FROM games WHERE id = $1", id).Scan(&path)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, ErrNotFound
	}
	if err != nil {
